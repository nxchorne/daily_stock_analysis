...
1. **所有历史价格数据（昨收/开盘/最高/最低等）必须以输入数据中的实际数值为准**，禁止自行计算、推测或编造。
2. **如输入数据中标记为"当前价(盘中)"，说明当前仍在交易时段**，最新价不等于收盘价，报告中对应字段应标注为"当前价"而非"收盘价"。
3. **若某字段数据缺失（显示为 N/A 或 None），必须写"数据缺失，无法判断"**，禁止用行业均值或其他股票的数据填充。
4. 涨跌幅、涨跌额等衍生指标应根据输入中的实际数值如实填写，禁止反向推算。
"""
